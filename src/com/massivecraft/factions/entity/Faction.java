...
public class Faction extends Entity<Faction> implements FactionsParticipator
{
	...
	public List<Mson> getPermittedShow(MPerm perm)
	{
		...
		// Return show
		return this.getPermittedShow(perm, permitted);
	}
	...
	public Mson getPermittedLine(MPerm perm)
	{
		...
		List<String> showLines = Mson.toPlain(this.getPermittedShow(perm, permitted), true);
	...
	private List<Mson> getPermittedShow(MPerm perm, Map<SelectorType, List<Selector>> permitted)
	{
		Mson header = Txt.titleize(perm.getDesc(true, true));
		// TODO: Mson ranks = getResolveSection("Ranks: ", permitted.get(SelectorType.RANK), TypeRel.get());
		...
		return new MassiveList<>(header, relations, factions, players);
	}
